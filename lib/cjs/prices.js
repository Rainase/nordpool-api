...
const DayAheadPricesHourly = ({ currency, area, vat, endDate }) => __awaiter(void 0, void 0, void 0, function* () {
    const today = new Date();
    const fallbackDate = `${today.getDate()}-${today.getMonth()}-${today.getFullYear()}`;
    const finalDate = endDate ? endDate : fallbackDate;
    const url = `${config_1.config.priceUrlHourly}?currency=${currency},${currency},EUR,EUR&endDate=${finalDate}`;
